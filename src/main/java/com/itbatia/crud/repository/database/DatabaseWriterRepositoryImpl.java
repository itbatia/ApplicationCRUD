...
public class DatabaseWriterRepositoryImpl implements WriterRepository {
    ...
    public Writer getById(Integer integer) {
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                System.out.println("Writer " + integer + " doesn't exist");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    public Writer save(Writer writer) {
        ...
        if (writer.getPosts() != null) {
            writer.getPosts().forEach(post -> {
                databasePostRepository.save(post);
                saveWriterIdInTablePost(writer.getId(), post);
            });
        }
        return writer;
    }
    ...
    private void updatePostsByWriterID(Writer writer) {
        deletePostsByWriterId(writer.getId());
        writer.getPosts().forEach(post -> {
            databasePostRepository.save(post);
            saveWriterIdInTablePost(writer.getId(), post);
        });
